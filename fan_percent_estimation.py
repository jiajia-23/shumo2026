...
class DWTSAdvAnalysis:
    ...
    def plot_season_elimination_matrix(self, estimation_results=None, save=True):
        """
        Matrix plot: Season (Y) x Week (X) showing elimination/exit status.

        Status rules (per week):
        - Withdrew: any exited contestant has results containing "Withdrew"
        - Eliminated: exited contestant(s) and none withdrew
        - No Elimination: week exists in data but exit count == 0
        """
        ...
        data['is_exited'] = data['is_exited'].astype(bool)

        max_season = int(max(34, data['season'].max()))
        max_week = int(data['week'].max())
        seasons_all = list(range(1, max_season + 1))
        weeks_all = list(range(1, max_week + 1))

        # Summarize status per (season, week)
        summary_rows = []
        for (season, week), group in data.groupby(['season', 'week']):
            results_str = group['results'].astype(str)
            withdrew_mask = results_str.str.contains('Withdrew', case=False, na=False) & group['is_exited']
            withdrew_count = int(withdrew_mask.sum())
            exit_count = int(group['is_exited'].sum())
            eliminated_count = max(exit_count - withdrew_count, 0)
        ...
        # Background matrix to show weeks that exist in data
        base_matrix = pd.DataFrame(0, index=seasons_all, columns=weeks_all, dtype=float)
        for _, row in summary_df.iterrows():
            if row['season'] in base_matrix.index and row['week'] in base_matrix.columns:
                base_matrix.loc[row['season'], row['week']] = 1.0

        color_grid = '#F3F3F3'
        color_elim_1 = '#8B1E3F'  # muted red / burgundy
        color_elim_2 = '#A63A53'
        color_elim_3 = '#C25B6B'
        color_withdrew = '#D97A1E'  # burnt orange / amber
        color_safe = '#2E8B57'      # forest green
        color_text = '#222222'

        marker_map = {
            'elim_1': ('X', color_elim_1, color_elim_1, 110),
            'elim_2': ('P', color_elim_2, color_elim_2, 110),
            'elim_3': ('s', color_elim_3, color_elim_3, 110),
            'withdrew': ('^', color_withdrew, color_withdrew, 110),
            'no_elim': ('D', color_safe, 'none', 110),
        }
        ...
        with plt.style.context('seaborn-v0_8-whitegrid'):
            ...
            sns.heatmap(
                base_matrix,
                ax=ax,
                cmap=sns.color_palette(['#FFFFFF', color_grid]),
                cbar=False,
                linewidths=0.6,
                linecolor='#E1E1E1'
            )

            # Use heatmap cell centers (0.5 offset) for scatter placement
            def to_center(val):
                return val - 0.5

            # Plot: no elimination
            no_elim = summary_df[summary_df['exit_count'] == 0]
            if not no_elim.empty:
                m, edge, face, size = marker_map['no_elim']
                ax.scatter(
                    no_elim['week'].map(to_center),
                    no_elim['season'].map(to_center),
                    marker=m,
                    s=size,
                    edgecolors=edge,
                    facecolors=face,
            ...
            # Plot: withdrew only
            withdrew_only = summary_df[(summary_df['withdrew_count'] > 0) & (summary_df['eliminated_count'] == 0)]
            if not withdrew_only.empty:
                m, edge, face, size = marker_map['withdrew']
                ax.scatter(
                    withdrew_only['week'].map(to_center),
                    withdrew_only['season'].map(to_center),
                    marker=m,
                    s=size,
                    edgecolors=edge,
                    facecolors=face,
                ...
                for _, row in withdrew_only.iterrows():
                    if row['withdrew_count'] > 1:
                        ax.text(
                            to_center(row['week']) + 0.18,
                            to_center(row['season']) + 0.18,
                            str(int(row['withdrew_count'])),
                            fontsize=9,
                            color=color_text,
                            ha='left',
                            va='bottom',
            ...
            # Plot: elimination only (1/2/3+)
            elim_only = summary_df[(summary_df['eliminated_count'] > 0) & (summary_df['withdrew_count'] == 0)]
            for count, key in [(1, 'elim_1'), (2, 'elim_2'), (3, 'elim_3')]:
                subset = elim_only[elim_only['eliminated_count'] == count]
                if subset.empty:
                    continue
                m, edge, face, size = marker_map[key]
                ax.scatter(
                    subset['week'].map(to_center),
                    subset['season'].map(to_center),
                    marker=m,
                    s=size,
                    edgecolors=edge,
                    facecolors=face,
            ...
            elim_3p = elim_only[elim_only['eliminated_count'] > 3]
            if not elim_3p.empty:
                m, edge, face, size = marker_map['elim_3']
                ax.scatter(
                    elim_3p['week'].map(to_center),
                    elim_3p['season'].map(to_center),
                    marker=m,
                    s=size,
                    edgecolors=edge,
                    facecolors=face,
                ...
                for _, row in elim_3p.iterrows():
                    ax.text(
                        to_center(row['week']) + 0.18,
                        to_center(row['season']) + 0.18,
                        str(int(row['eliminated_count'])),
                        fontsize=9,
                        color=color_text,
                        ha='left',
                        va='bottom',
            ...
            # Plot: withdrew + eliminated (offset to avoid overlap)
            mixed = summary_df[(summary_df['withdrew_count'] > 0) & (summary_df['eliminated_count'] > 0)]
            if not mixed.empty:
                offset = 0.15
                m_w, edge_w, face_w, size_w = marker_map['withdrew']
                ax.scatter(
                    mixed['week'].map(lambda w: to_center(w) - offset),
                    mixed['season'].map(to_center),
                    marker=m_w,
                    s=size_w,
                    edgecolors=edge_w,
                    facecolors=face_w,
                ...
                for _, row in mixed.iterrows():
                    if row['eliminated_count'] == 1:
                        key = 'elim_1'
                    elif row['eliminated_count'] == 2:
                        key = 'elim_2'
                    else:
                        key = 'elim_3'
                    m_e, edge_e, face_e, size_e = marker_map[key]
                    ax.scatter(
                        to_center(row['week']) + offset,
                        to_center(row['season']),
                        marker=m_e,
                        s=size_e,
                        edgecolors=edge_e,
                        facecolors=face_e,
                    ...
                    if row['eliminated_count'] > 2:
                        ax.text(
                            to_center(row['week']) + offset + 0.18,
                            to_center(row['season']) + 0.18,
                            str(int(row['eliminated_count'])),
                            fontsize=9,
                            color=color_text,
            ...
            ax.set_title('Season-Week Elimination Matrix', fontsize=16, pad=12)

            ax.set_xticks([w - 0.5 for w in weeks_all])
            ax.set_xticklabels(weeks_all, fontsize=10)
            ax.set_yticks([s - 0.5 for s in seasons_all])
            ax.set_yticklabels(seasons_all, fontsize=10)

            ax.set_xlim(0, max_week)
            ax.set_ylim(max_season, 0)

            from matplotlib.lines import Line2D
            from matplotlib.legend_handler import HandlerTuple
            h_elim1 = Line2D([0], [0], marker='X', color='none', label='Eliminated (1)',
                             markerfacecolor=color_elim_1, markeredgecolor=color_elim_1, markersize=9)
            h_elim2 = Line2D([0], [0], marker='P', color='none', label='Eliminated (2)',
                             markerfacecolor=color_elim_2, markeredgecolor=color_elim_2, markersize=9)
            h_elim3 = Line2D([0], [0], marker='s', color='none', label='Eliminated (3)',
                             markerfacecolor=color_elim_3, markeredgecolor=color_elim_3, markersize=9)
            h_withdrew = Line2D([0], [0], marker='^', color='none', label='Withdrew',
                                markerfacecolor=color_withdrew, markeredgecolor=color_withdrew, markersize=9)
            h_no = Line2D([0], [0], marker='D', color='none', label='No Elimination',
                          markerfacecolor='none', markeredgecolor=color_safe, markersize=9)

            h_mix1 = (Line2D([0], [0], marker='^', color='none', markerfacecolor=color_withdrew,
                             markeredgecolor=color_withdrew, markersize=8),
                      Line2D([0], [0], marker='X', color='none', markerfacecolor=color_elim_1,
                             markeredgecolor=color_elim_1, markersize=8))
            h_mix2 = (Line2D([0], [0], marker='^', color='none', markerfacecolor=color_withdrew,
                             markeredgecolor=color_withdrew, markersize=8),
                      Line2D([0], [0], marker='P', color='none', markerfacecolor=color_elim_2,
                             markeredgecolor=color_elim_2, markersize=8))

            ax.legend(
                handles=[h_elim1, h_elim2, h_elim3, h_withdrew, h_mix1, h_mix2, h_no],
                labels=['Eliminated (1)', 'Eliminated (2)', 'Eliminated (3)',
                        'Withdrew', 'Withdrew + 1 Elim', 'Withdrew + 2 Elim', 'No Elimination'],
            ...
            if save:
                out_path = os.path.join(PIC_ROOT, 'season_week_elimination_matrix.png')
                plt.savefig(out_path, dpi=300, bbox_inches='tight')
                print(f"Plot saved to: {out_path}")
            else:
                plt.show()
...
def main():
    ...
    analyzer.plot_season_elimination_matrix(estimation_results=estimator.estimation_results, save=True)   
    print("\n" + "=" * 60)
